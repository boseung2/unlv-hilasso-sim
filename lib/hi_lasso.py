...
def _fit_adaptive(Xs, ys, weights, cv, rs):
    """adaptive LASSO: 열을 1/weight로 스케일 → LASSO → 계수 역스케일."""
    Xa = Xs / weights
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", ConvergenceWarning)
...
def fit_hi_lasso(X, y, q1="auto", q2="auto", L=30, alpha=0.05,
    ...
    선택 = 이항 유의성 검정(p<alpha) — 임계값 튜닝 불필요."""
    ...
    pi = d.sum() / max(int(not_null.sum()), 1)              # 전체 nonzero율
    p_values = binom.sf(d - 1, n=B2, p=pi)
    coef_ = np.where(p_values < alpha, b2_mean, 0.0)
